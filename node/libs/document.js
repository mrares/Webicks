//var Script = process.binding('evals').Script;
var vm = require('vm');

var stats = {};

var document = module.exports = function(stats) {
    console.log('creating document');
    stats = stats;
};

document.prototype.response = {};
document.prototype.stats = {};
document.prototype.cache = [];
document.prototype.load = function(location) {
    console.log('Loading from: ' + location);
    if(this.cache[location] != undefined) {
        console.log('Fetching document from cache.');
        stats.hits+=1;
        var sandbox = {
            console:console,
            redis: this.redis,
            response: this.response,
            result:{},
            onSuccess:function(value){
                response.write(value);
                response.end();
            },
            onFail:function(){
                response.write(value);
                response.end();
            }
        };
        //script = new Script(value);
//        console.log(this.cache[location].toString());
        this.cache[location].runInNewContext(sandbox);
    } else {
        stats.misses+=1;
        console.log('Fetching document from redis.');
        this.redis.get(
            location,
            (function( err, value ){
                if(err != null) {
                    console.log('Got error from redis.');
                    consle.log(err);
                    response.write(500);
                    response.end();
                }
                if(value == null) {
                    console.log('Document not found in redis.');
                    response.write("404\n");
                    response.end();
                } else {
                    console.log('OK!');
                    var sandbox = { 
                        console:console, 
                        redis: this.redis,
                        response: this.response, 
                        result:{}, 
                        onSuccess:function(value){
                            response.write(value);
                            response.end();
                        }, 
                        onFail:function(){
                            response.write(value);
                            response.end();
                        }
                    };
                    try {
//                        console.log('RUNNING SCRIPT FROM ' + location);
//                        console.log('========================================================');
//                        console.log(value);
                        var scriptString = value.toString('utf8', 0, value.length - 1 );
                        console.log(value);
                        console.log(scriptString.toString());
                        console.log(scriptString);
//                        console.log('========================================================');
//                        this.cache[location] = value;
                        this.cache[location] = vm.createScript(scriptString);
//                        console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
//                        console.log(script);
                        this.cache[location].runInNewContext(sandbox);
                    } catch (e) {
                        console.log(e);
                        console.log(e.message);
                        console.log('theres been an issue');
                        // TODO: handle exception
                        this.response.write("500\n");
                        this.response.end();
                    }
//                    setTimeout((function(){delete this.cache[location];}).bind(this),1000);
                }
            }).bind(this)
        );
    }

    
    
    
    
    
};
document.prototype.save = function(location, content) {
    return true;
};